...
struct Model{
    ...
    private:
        void init_vocab(std::string vocab_path = "/src/projectX/model/bert_cased_vocab.txt"){
            std::tie(token2id, id2token) = get_vocab(vocab_path);
        }



        void init_bert(std::string bert_path = "/src/projectX/model/traced_text_classification_model.pt"){
